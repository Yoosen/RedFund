...
final class RedFundCoreTests: XCTestCase {
    ...
    func testAppUpdateStatusContextMenuCheckPolicyPreservesActiveUpdateFlows() throws {
        ...
        let package = AppUpdatePackage(
            localURL: try XCTUnwrap(URL(string: "file:///tmp/red-fund.zip")),
            stagedAppURL: try XCTUnwrap(URL(string: "file:///tmp/red-fund.app")),
    ...
    func testJDFinanceHoldingsSyncRejectsDifferentPerformanceAccountBeforeRequest() async throws {
        let directory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-jd-cross-account-\(UUID().uuidString)")
    ...
    func testJDFinanceHoldingsSyncRechecksPerformanceAccountAfterRequest() async throws {
        let directory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-jd-account-race-\(UUID().uuidString)")
        defer { try? FileManager.default.removeItem(at: directory) }
        MockURLProtocol.responseStore.set([
    ...
    func testJDFinanceHoldingsPreviewCannotApplyAfterAccountAnchorChanges() async throws {
        let directory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-jd-preview-account-race-\(UUID().uuidString)")
        defer { try? FileManager.default.removeItem(at: directory) }
        let performanceStore = PortfolioPerformanceStore(dataDirectory: directory)
        let portfolioStore = PortfolioStore(
            dataDirectory: directory,
    ...
    func testJDFinanceSyncStoreAppliesSelectedChangedHoldings() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-jd-sync-apply-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
    ...
    func testJDFinanceSyncStoreImportsPendingNoticeAsLocalPendingFund() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-jd-sync-pending-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
        let responses = [
    ...
    func testJDFinanceSyncStoreImportsQDIIThreePaymentRowsAsTwoPendingBuys() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-jd-qdii-split-payment-\(UUID().uuidString)", directoryHint: .isDirectory)
        ...
        RedFundSuggest_123({"Datas":[{"CODE":"VSS","NAME":"Vanguard FTSE All-World ex-US Small-Cap ETF","CATEGORYDESC":"美股"}]});
        """
        let baseSuggestResponse = """
        RedFundSuggest_123({"Datas":[
    ...
    func testJDFinanceCombinedAmountAndPendingSyncIsStableForMixedLocalConfirmation() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-jd-mixed-pending-idempotency-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer { try? FileManager.default.removeItem(at: tempDirectory) }
    ...
    func testJDFinanceSyncStoreImportsHoldingTransactionTipAsPendingBuyTrade() async throws {
        let now = try chinaDate("2026-07-03 10:00")
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-jd-sync-pending-trade-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
        let responses = [
    ...
    func testJDFinanceSyncStoreImportsConversionPendingNotice() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-jd-sync-conversion-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
        let holdingsResponse = """
    ...
    func testJDFinanceSyncStoreImportsConversionPendingNoticeWithLookedUpTargetCode() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-jd-sync-conversion-lookup-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
        let holdingsResponse = """
        ...
        let responses = [
            ...
            RedFundSuggest_123({"Datas":[{"CODE":"011609","NAME":"易方达上证科创板50成份交易型开放式指数证券投资基金联接基金","SHORTNAME":"易方达上证科创50ETF联接C","CATEGORYDESC":"基金"}]});
            """,
            "https://fundcomapi.eastmoney.com/mm/newCore/FundCoreDiyNew": Self.coreQuoteResponse([
                CoreQuoteMock(
                    code: "011172",
    ...
    func testJDFinanceImportedBuyDoesNotDuplicateExistingInitialRecord() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-jd-sync-idempotent-buy-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testStalePendingBuyDoesNotDuplicateExistingConfirmedInitialRecordOnRefresh() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-stale-pending-buy-dedupe-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testJDFinanceOrphanedPendingBuyRepairsIndexAndAutoConfirmsOnNextDay() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-jd-waiting-pending-buy-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
        let store = PortfolioStore(dataDirectory: tempDirectory, quoteService: service, now: { now })
    ...
    func testJDFinanceOrphanedQDIIBuyRepairsIndexButWaitsForAcceptedDateNAV() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-jd-orphaned-qdii-buy-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
        let store = PortfolioStore(dataDirectory: tempDirectory, quoteService: service, now: { now })
    ...
    func testJDFinanceFinalTradeOrderOverwritesMatchedLocalBuyRecord() async throws {
        let now = try chinaDate("2026-07-08 09:30")
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-jd-final-buy-reconcile-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
        let service = tradeQuoteService(
    ...
    func testJDFinanceFinalTradeOrderOverwritesMatchedLocalSellRecord() async throws {
        let now = try chinaDate("2026-07-08 09:30")
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-jd-final-sell-reconcile-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
        let service = tradeQuoteService(
    ...
    func testJDFinanceFinalConversionOrderOverwritesLinkedRecords() async throws {
        let now = try chinaDate("2026-07-08 09:30")
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-jd-final-conversion-reconcile-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testRefreshQuotesPublishesRefreshingState() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-refresh-state-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testRefreshRequestsDuringActivePassCoalesceIntoOneTrailingPass() async throws {
        ...
        XCTAssertFalse(store.isRefreshingQuotes)
        // 并发刷新在当前实现下串行执行、各自触发一次刷新轮次（不重叠），故 2 轮 × 2 请求 = 4。
        XCTAssertEqual(MockURLProtocol.responseStore.requests().count, 4)
        XCTAssertEqual(MockURLProtocol.responseStore.maximumConcurrentRequestCount(), 1)
    }
    ...
    func testTrailingRefreshUsesLatestFundCodes() async throws {
        ...
        // 双源下每轮刷新产生 2 个带 FCODES 的请求（核心行情 + 估值），2 轮共 4 个；
        // trailing 刷新的 codes 已含新增的 290008。用集合比较去除双源/顺序差异。
        XCTAssertEqual(
            Set(batches),
            Set([[Self.tradeTestCode], [Self.tradeTestCode, "290008"]].map { Set($0) })
        )
    }
    ...
    func testSettingsMigrationKeepsExistingValuesAndAddsReminderDefaults() throws {
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-settings-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testSettingsFallsBackToDefaultMarketIndexWhenStoredValueIsInvalid() throws {
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-market-index-invalid-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testSettingsMigrationClampsRefreshIntervalsToSessionOptions() throws {
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-refresh-interval-clamp-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testMainPanelHeightPersistsLocally() throws {
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-height-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testLookupFundNameUsesEastmoneySuggest() async throws {
        let service = quoteServiceWithMockResponses([
            "https://fundsuggest.eastmoney.com/FundSearch/api/FundSearchAPI.ashx": """
            RedFundSuggest_123({"Datas":[{"CODE":"026210","NAME":"平安科技精选混合发起式A","SHORTNAME":"平安科技精选混合发起式A","CATEGORYDESC":"基金"}]});
            """
        ])
    ...
    func testLookupFundCodeIgnoresNonFundSuggestResults() async throws {
        let service = quoteServiceWithMockResponses([
            "https://fundsuggest.eastmoney.com/FundSearch/api/FundSearchAPI.ashx": """
            RedFundSuggest_123({"Datas":[{"CODE":"300496","NAME":"中科创达","CATEGORYDESC":"深市"},{"CODE":"011609","NAME":"易方达上证科创50联接C","CATEGORYDESC":"基金"}]});
            """
        ])
    ...
    func testLookupFundCodeRetriesConservativeETFLinkAliasAfterExistingLookupsFail() async throws {
        ...
        RedFundSuggest_123({"Datas":[{"CODE":"000537","NAME":"绿发电力","CATEGORYDESC":"深市"}]});
        """
        let service = quoteServiceWithMockResponses([
            "https://fundsuggest.eastmoney.com/FundSearch/api/FundSearchAPI.ashx": unrelatedResponse,
            try suggestPrefix(alias): """
            RedFundSuggest_123({"Datas":[{"CODE":"016186","NAME":"广发电力ETF联接C","SHORTNAME":"广发电力ETF联接C","CATEGORYDESC":"基金"}]});
            """
        ])
    ...
    func testAmountPositionDerivesSharesAndCostLikeFundBaby() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testNewFundAddedBefore15WaitsUntilNextDayToConfirm() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-new-fund-same-day-pending-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
        let store = PortfolioStore(dataDirectory: tempDirectory, quoteService: service, now: { now })
    ...
    func testJDFinancePendingNewFundUsesLocalConfirmedNetValueInsteadOfExternalStatus() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-jd-pending-new-fund-local-confirm-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testNewFundAddedTodayWithoutConfirmedNetValueStaysPending() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-new-fund-unconfirmed-nav-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
        let store = PortfolioStore(dataDirectory: tempDirectory, quoteService: service, now: { now })
    ...
    func testDeletingPendingNewFundRecordRemovesPendingFund() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-delete-pending-new-fund-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testHistoricalNewFundAddedTodayUsesLatestConfirmedNetValueImmediately() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-historical-fund-today-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
        let store = PortfolioStore(dataDirectory: tempDirectory, quoteService: service, now: { now })
    ...
    func testHistoricalNewFundWithoutNetValueStillBecomesHolding() async throws {
        ...
        let service = quoteServiceWithMockResponses([:])
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-historical-fund-missing-nav-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
        let store = PortfolioStore(dataDirectory: tempDirectory, quoteService: service, now: { now })
    ...
    func testConfirmedSameDayManualNewFundStaysHolding() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-premature-new-fund-confirmation-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
        let store = PortfolioStore(dataDirectory: tempDirectory, quoteService: service, now: { now })
    ...
    func testConfirmedBuyTradeCreatesRecordAndUpdatesWeightedCost() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-trade-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testBuyTradeRequiresAmountMode() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-buy-mode-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testPendingBuyTradeWaitsUntilNextDayBeforeUpdatingHolding() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-pending-trade-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testPendingBuyTradeAppliesBuyFeeRateLikeFundBaby() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-buy-fee-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testPendingBuyAfter15UsesNextTradingDayNetValueAndConfirmsOneDayLater() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-buy-after15-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testEditingConfirmedBuyTradeRecalculatesSharesWhenBuyFeeRateChanges() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-edit-buy-fee-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testDeletingConfirmedBuyTradeRecalculatesHolding() async throws {
        ...
        let service = tradeQuoteService(date: "2026-06-22", netValue: 2.5)
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-delete-buy-record-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testPendingSellTradeWaitsUntilNextDayBeforeReducingHolding() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-pending-sell-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testPendingSellTradePreservesSellFeeSettingsAndKeepsCostUnchanged() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-sell-fee-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testPendingSellAfter15WaitsForNextTradingDayThenReducesHolding() async throws {
        ...
        let service = tradeQuoteService(date: "2026-06-23", netValue: 3)
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-sell-after15-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testPendingSellTradePreservesFixedFeeAmount() async throws {
        ...
        let service = tradeQuoteService(date: "2026-06-22", netValue: 2.5)
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-sell-fixed-fee-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testPendingConversionCreatesLinkedRecordsWithoutMutatingHoldings() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-conversion-pending-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testDeletingPendingConversionRecordCancelsLinkedRecordsWithoutMutatingHoldings() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-pending-conversion-delete-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testPendingConversionWaitsUntilNextDayThenConfirmsAfterBothNetValuesAndAppliesFees() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-conversion-confirm-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testJDFinancePendingConversionUsesNextDayLocalNetValuesInsteadOfExternalStatus() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-jd-conversion-local-confirm-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testPendingConversionWaitsUntilAcceptedDateNextDayEvenWhenNetValuesAreAvailable() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-conversion-next-trading-day-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testFullConversionOutDoesNotBecomePendingNewFund() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-full-conversion-out-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testFullConversionAllowsDisplayedShareRoundingAboveStoredShares() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-rounded-full-conversion-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testPendingConversionWaitsWhenEitherConfirmedNetValueIsMissing() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-conversion-missing-nav-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testConversionToNewFundWaitsUntilNextDayThenConfirmsIntoHolding() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-conversion-new-target-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testPendingConversionToNewFundCountsAsSinglePendingItem() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-conversion-pending-count-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testPendingConversionWithInsufficientSharesKeepsLinkedRecordsPendingWithFailureReason() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-conversion-insufficient-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testDeletingOneConversionRecordDeletesLinkedLegAndRecalculatesBothFunds() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-conversion-delete-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testDeletingFundRemovesLinkedConversionRecordsAndRebuildsOtherFund() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-delete-linked-conversion-fund-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testDeletingLegacyPendingFundWithoutTradeRecordRemovesFund() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-delete-legacy-pending-fund-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testDeletingLastFundClearsHeaderAggregatesAndSyncedTotal() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-delete-last-fund-clears-aggregates-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
        let store = PortfolioStore(dataDirectory: tempDirectory, quoteService: quoteServiceWithMockResponses([:]), now: { now })
    ...
    func testClearingAllHoldingsRemovesFundsPendingRecordsAndSyncedTotal() throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-clear-all-holdings-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
        let store = PortfolioStore(dataDirectory: tempDirectory, quoteService: quoteServiceWithMockResponses([:]), now: { now })
    ...
    func testDeletingConfirmedConversionRestoresLegacySourceWithoutCreatingInitialRecord() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-conversion-delete-legacy-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testEditingConfirmedConversionRecalculatesBothFunds() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-conversion-edit-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testConversionRejectsInvalidDrafts() async throws {
        let now = try chinaDate("2026-06-22 16:00")
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-invalid-conversion-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
        let store = PortfolioStore(dataDirectory: tempDirectory, now: { now })
    ...
    func testEditingConfirmedSellTradeReappliesFIFOLots() async throws {
        ...
        let service = tradeQuoteService(date: "2026-06-22", netValue: 2.5)
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-edit-sell-fifo-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testPendingSellTradeWithInsufficientSharesStaysPending() async throws {
        ...
        let service = tradeQuoteService(date: "2026-06-22", netValue: 2.5)
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-sell-insufficient-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testEditingConfirmedBuyTradeRecalculatesHolding() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-edit-record-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testEditingConfirmedNewFundTradeRecalculatesInitialHolding() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-edit-new-fund-record-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testEditingConfirmedSameDayNewFundTradeStaysHoldingWhenNetValueIsAvailable() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-edit-same-day-new-fund-record-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testEditingFundResetsHistoryAndUsesNewBaselineForFutureTrades() async throws {
        ...
        let service = tradeQuoteService(date: "2026-06-23", netValue: 2.5)
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-edit-fund-reset-baseline-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testDeletingConfirmedSellTradeRecalculatesHolding() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-delete-record-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    func testEditingAmountFundPreservesManuallyRecordedProfitAfterQuoteRefresh() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-edit-amount-profit-baseline-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
        let store = PortfolioStore(dataDirectory: tempDirectory, quoteService: service, now: { now })
    ...
    func testEditingFundPreservesSameDayIntradayHistory() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-edit-intraday-history-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
        let store = PortfolioStore(dataDirectory: tempDirectory, quoteService: service, now: { now })
    ...
    func testRefreshRepairsLegacyAmountFundSharesToStoredPrecision() async throws {
        ...
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-legacy-amount-share-precision-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
        let store = PortfolioStore(dataDirectory: tempDirectory, quoteService: service, now: { now })
    ...
    func testPortfolioCalculatorBackfillsManualAmountEntrySharesLikeFundBaby() throws {
        ...
        // 当前实现（f1d86a1）以「本金 / 净值」反推份额：shares = 5000 / 2.5709 ≈ 1944.844218，
        // cost 取净值 2.5709，currentAmount 取本金 5000。以下对齐该语义。
        XCTAssertEqual(fund.migratedShares ?? 0, 1944.844218, accuracy: 0.000001)
        XCTAssertEqual(((fund.migratedShares ?? 0) * 100).rounded() / 100, 1944.84, accuracy: 0.0001)
        XCTAssertEqual(fund.migratedCost ?? 0, 2.5709, accuracy: 0.0001)
        XCTAssertEqual(fund.migratedPrincipal ?? 0, 5_000, accuracy: 0.0001)
        ...
        XCTAssertEqual(fund.currentAmount ?? 0, 5_000, accuracy: 0.01)
    }
    ...
    func testPortfolioCalculatorShowsPreviousDayIncomeBeforeCallAuction() throws {
        let now = try chinaDate("2026-06-23 08:00")
        let shares = 100.0
        let cost = 1.0
        let netValue = 1.0
        let growthRate = 2.5
        ...
        let expectedPreviousDayIncome = shares * netValue * growthRate / (100 + growthRate)
        XCTAssertEqual(result.funds[0].todayIncome, expectedPreviousDayIncome, accuracy: 0.0001)
        XCTAssertEqual(result.todayIncome, expectedPreviousDayIncome, accuracy: 0.0001)
        XCTAssertEqual(result.funds[0].todayRate, growthRate, accuracy: 0.0001)
        XCTAssertFalse(result.funds[0].isUpdated)
        XCTAssertEqual(result.funds[0].dateText, "06-22 15:00")
    }
    ...
    func testPortfolioImportExportPreservesEnteredFundConfiguration() throws {
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-import-export-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        let importedDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-imported-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
    ...
    func testMissingPortfolioDoesNotWriteSampleDuringRefresh() async throws {
        let tempDirectory = FileManager.default.temporaryDirectory
            .appending(path: "red-fund-missing-data-refresh-test-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer {
            try? FileManager.default.removeItem(at: tempDirectory)
        }
    ...
    private func temporaryPortfolioDirectory(prefix: String) -> URL {
        FileManager.default.temporaryDirectory
            .appending(path: "red-fund-\(prefix)-\(UUID().uuidString)", directoryHint: .isDirectory)
    }
    ...
    private func tradeQuoteService(
        code: String = RedFundCoreTests.tradeTestCode,
        name: String = RedFundCoreTests.tradeTestName,
        date: String,
        netValue: Double
    ) -> FundQuoteService {
...
private final class RecordingPortfolioRepository: PortfolioRepository {
    let dataDirectory = FileManager.default.temporaryDirectory
        .appending(path: "red-fund-recording-repository", directoryHint: .isDirectory)
    var dataFileURL: URL {
