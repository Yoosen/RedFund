...
MODE="${1:-run}"
EXECUTABLE_NAME="RedFund"
APP_NAME="Red Fund"
BUNDLE_ID="com.yoosen.red-fund.swift"
MIN_SYSTEM_VERSION="14.0"
BUILD_CONFIGURATION="${RED_FUND_BUILD_CONFIGURATION:-debug}"
...
RESOURCE_BUNDLE_NAME="RedFund_RedFund.bundle"
...
case "$BUILD_CONFIGURATION" in
  ...
  release)
    ;;
  *)
    echo "error: RED_FUND_BUILD_CONFIGURATION must be debug or release" >&2
    exit 2
    ;;
esac
...
chmod +x "$APP_BINARY"
if [[ -f "$APP_ICON" ]]; then
  cp "$APP_ICON" "$APP_RESOURCES/RedFund.icns"
fi
