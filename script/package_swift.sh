...
SIGN_IDENTITY="${RED_FUND_SIGN_IDENTITY:-}"
NOTARY_PROFILE="${RED_FUND_NOTARY_PROFILE:-red-fund}"
SKIP_NOTARY="${RED_FUND_SKIP_NOTARY:-0}"
SKIP_DMG_LAYOUT="${RED_FUND_SKIP_DMG_LAYOUT:-0}"
SIGNING_KIND="custom"
...
if [[ -z "$SIGN_IDENTITY" ]]; then
  if [[ "${RED_FUND_ALLOW_ADHOC:-0}" == "1" ]]; then
    ...
    SKIP_NOTARY="1"
    cat >&2 <<'EOF'
warning: RED_FUND_ALLOW_ADHOC=1 set; using ad-hoc signing.
...
Install an Apple Developer certificate, or set:
  RED_FUND_SIGN_IDENTITY="Developer ID Application: ..."
  RED_FUND_SIGN_IDENTITY="Apple Development: ..."
...
Set RED_FUND_ALLOW_ADHOC=1 to allow ad-hoc signing in CI only.
EOF
    exit 1
  fi
fi
...
Set RED_FUND_NOTARY_PROFILE to use a different profile name.
EOF
    exit 1
  fi
fi

RED_FUND_BUILD_CONFIGURATION=release "$ROOT_DIR/script/build_and_run.sh" --build
