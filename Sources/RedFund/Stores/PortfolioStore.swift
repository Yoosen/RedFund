    ...
    func performJDFinanceAtomicMutation(
        ...
        do {
            ...
            let stagingPerformanceDirectory = FileManager.default.temporaryDirectory
                .appending(
                    path: "red-fund-jd-staging-performance-\(UUID().uuidString)",
...
enum AppDataPaths {
    ...
    /// 首次启动时把旧 red-fund 数据目录整体迁移到 red-fund，避免用户数据丢失。
    ...
    private static func migrateLegacyDataDirectoryIfNeeded() {
        ...
        do {
            ...
            NSLog("AppDataPaths: 旧数据目录迁移失败（red-fund -> red-fund）: \(error.localizedDescription)")
        }
    }
